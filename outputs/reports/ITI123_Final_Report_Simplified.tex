...
\title{\textbf{ITI123 Final Project Report}\\\vspace{0.3cm}AI-Based Badminton Stroke Classification}
...
\begin{abstract}
This project built an AI system that watches a badminton video and identifies what type of shot was played. Three approaches were explored: First, body skeleton tracking (joint keypoints) was used to distinguish between Clear and Smash shots, but this failed because players had nearly identical body positions for both shots. Second, raw video frames were analysed using a neural network combining image recognition with motion tracking, achieving 75.3\% accuracy across five shot types with the MobileNet-LSTM model. Finally, a user-friendly web application was built where anyone could upload a video and receive instant shot analysis with confidence scores and warnings.

The key lesson was that the obvious approach did not work, and pivoting to what the data revealed was necessary. This project demonstrates the complete AI development process---from initial failure through successful implementation to real-world deployment.
\end{abstract}
...
\item \textbf{Phase 2 - The Successful Model:} Built a neural network that watches video frames and tracks motion over time. Result: 75.3\% accuracy across five shot types (Clear, Drive, Drop, Lift, Smash) using the MobileNet-LSTM model.

\item \textbf{Phase 3 - The User Application:} Created a web app called ``Shot Coach'' where users upload videos and get instant analysis with explanations and confidence scores. The app is publicly deployed on Hugging Face Spaces and the full project source is available on GitHub (see Section~\ref{sec:deployment} for URLs).
\end{enumerate}
...
\midrule
Drop & 7,549 & 33.8\% & Majority class \\
Lift & 4,806 & 21.5\% & Well-represented \\
Drive & 3,683 & 16.5\% & Moderate \\
Smash & 3,602 & 16.2\% & Moderate \\
Clear & 2,662 & 11.9\% & Minority class \\
\midrule
\textbf{Total} & 22,302 & 100\% & Imbalanced (2.8:1 ratio) \\
\bottomrule
\end{tabular}
\end{table}

The biggest challenge? \textbf{Class imbalance}---I had 2.8 times more Drop shots than Clears. This means the AI might become biased toward predicting Drop shots more often.
...
\subsection{Results: 74.6\% Accuracy (ResNet18-BiLSTM Baseline)}\label{sec:resnet-results}

\textit{Note: These results are for the ResNet18-BiLSTM model. A lighter MobileNet-LSTM model trained subsequently achieved 75.3\% accuracy and was selected for deployment; see Section~\ref{sec:model-arch}.}
...
\begin{table}[H]
\centering
\caption{Confusion Matrix for ResNet18-BiLSTM (What Gets Confused with What)}
...
\item \textbf{Two-stage training:} Training in stages (first image recognition, then motion tracking) meaningfully improved accuracy over end-to-end training from scratch
\end{enumerate}

\subsection{Training History: ResNet18-BiLSTM}

Figure~\ref{fig:history_resnet} shows the loss and accuracy curves over 44 epochs of training. The gap between training and validation accuracy in later epochs reflects mild overfitting, which early stopping contained.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{training_history_resnet.png}
\caption{ResNet18-BiLSTM training history (44 epochs). Left: cross-entropy loss; Right: accuracy. Validation accuracy plateaued around epoch 35, triggering early stopping at epoch 44. Final test accuracy: 74.6\%.}
\label{fig:history_resnet}
\end{figure}

\section{Model Architectures: ResNet18-BiLSTM and MobileNet-LSTM}\label{sec:model-arch}
...
\begin{itemize}
  ...
  \item \textbf{Two-stage training:} The CNN backbone was first frozen while the LSTM head trained, then the entire network was fine-tuned end-to-end. This prevented the pre-trained weights from being corrupted early in training and meaningfully improved final accuracy compared to end-to-end training from the start.
  \item \textbf{Focal Loss with class weights:} Addressed the 2.8:1 class imbalance (Drop vs.\ Clear) by penalising mistakes on minority classes more heavily. Clear shots received a weight of $2.9\times$ relative to Drop shots.
\end{itemize}
...
Adaptive avg pool & $7\times 7 \to 1\times 1$ & Spatial aggregation \\
...
\textbf{Model size} & \multicolumn{2}{l}{\textbf{$\approx$17 MB}} \\
...
\subsubsection*{Training History: MobileNet-LSTM}

Figure~\ref{fig:history_mobilenet} shows the training curves over 56 epochs. MobileNet-LSTM trained for more epochs before early stopping, suggesting slower but steadier convergence relative to ResNet18-BiLSTM, and achieved a higher final test accuracy of 75.3\%.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{training_history_mobilenet.png}
\caption{MobileNet-LSTM training history (56 epochs). Left: cross-entropy loss; Right: accuracy. The model converged more gradually than ResNet18-BiLSTM and achieved a higher test accuracy of 75.3\%, making it the selected deployment model.}
\label{fig:history_mobilenet}
\end{figure}
...
Model size & 164 MB & $\approx$17 MB \\
...
MobileNet-LSTM was selected for the deployed Shot Coach application: it achieves higher test accuracy (75.3\% vs.\ 74.6\%) while using approximately 70\% fewer parameters, giving faster inference and a smaller deployment footprint. ResNet18-BiLSTM remains a valid alternative where bidirectional temporal context is preferred over model efficiency.
...
3. Model Inference & MobileNet-LSTM processes frames (\textless{}60ms on Mac M1) \\
...
\item \textbf{Model Inference:} PyTorch with trained MobileNet-LSTM
\item \textbf{Deployment:} Hugging Face Spaces (public cloud hosting, zero infrastructure required)
\item \textbf{Model Size:} $\sim$17 MB (stored via Git LFS in the HF Space repository)
\end{itemize}
...
The full project source code---training notebooks, model weights, and processing scripts---is available on GitHub at \url{https://github.com/paulgk/iti123_v2}. The repository \texttt{README.md} documents the project structure, lists all key files, and provides step-by-step instructions for running the application locally (\texttt{streamlit run badminton-shot/src/streamlit\_app.py}) and for reproducing the training from scratch using the Colab notebooks in \texttt{notebooks/}. All dependencies are pinned in \texttt{requirements.txt}, and the trained model weights are stored via Git LFS, ensuring a clean one-command local setup.
...
\midrule
\textbf{Accuracy} & 50.0\% & 75.3\% \\
\textbf{Improvement} & Baseline & +51\% relative \\
\textbf{Classes} & 2 (Clear, Smash) & 5 (all shot types) \\
\textbf{Features Used} & 60 engineered & 576 learned \\
\textbf{Captures Racket} & No & Yes (implicit) \\
\textbf{Captures Motion} & Yes (explicit) & Yes (implicit) \\
\textbf{Training Data} & 4,655 clips & 17,819 clips \\
\textbf{Inference Time} & 50ms & \textless{}60ms \\
\textbf{Model Size} & 5 MB & $\sim$17 MB \\
\bottomrule
\end{tabular}
\end{table}
...
\item Successful application of MobileNet-LSTM to badminton shot classification (75.3\% accuracy)
\item Effective handling of 2.8:1 class imbalance using Focal Loss
