...
\begin{abstract}
This milestone report presents a comprehensive investigation into classifying badminton Clear and Smash strokes using pose estimation techniques. We implemented MediaPipe-based pose extraction on the ShuttleSet dataset (4,682 forehand-only clips from 40 matches, with 301 backhand shots filtered out) and developed a machine learning pipeline with multiple model architectures. Despite implementing state-of-the-art feature engineering including novel wrist orientation features, comprehensive data preprocessing with group-stratified splitting, and proper padding masking for sequence models, all classifiers achieved approximately 50\% accuracy (random guessing level). Through rigorous Cohen's d effect size analysis, we identified the fundamental issue: Clear and Smash strokes are biomechanically indistinguishable at shuttlecock contact using pose-only features (Cohen's d < 0.2 across all 60 features). This report documents our methodology, extensive analysis, improvement iterations, and provides evidence-based recommendations for future work requiring multi-modal sensing or alternative stroke pair selection.
\end{abstract}
...
\textbf{Statistics (Forehand-Only Dataset)}:
\begin{itemize}
    \item Original clips: 4,983 (mixed forehand and backhand)
    \item \textbf{Filtered clips: 4,682} (forehand-only, extracted at shuttlecock contact)
    \item Backhand shots removed: 301 (291 Clear, 10 Smash)
    \item Clear strokes: 2,371 (50.6\%)
    \item Smash strokes: 2,311 (49.4\%)
    \item Unique matches: 40
    \item Pose extraction success rate: 99.4\% (4,655/4,682)
\end{itemize}
...
\textbf{Results (Forehand-Only Splits)}:
\begin{table}[H]
\centering
...
\midrule
\textbf{Overall} & \textbf{4,655} & \textbf{40} & \textbf{50.62\%} & \textbf{49.38\%} \\ \bottomrule
\end{tabular}
\end{table}

\textbf{Note}: 27 clips failed pose extraction (4,655 valid samples from 4,682 filtered clips).
